...
[!DNL :arrow_upper_right:] 请参阅[本节](https://experienceleague.adobe.com/docs/campaign-classic/using/getting-started/permissions/access-management.html?lang=zh-Hans#getting-started)了解如何将用户添加到 Adobe Campaign。
...
1. 导入用户档案和创建受众
   ...
   [!DNL :bulb:] [了解如何导入用户档案](import.md)。

   受众将分组到列表中，并可通过工作流创建。然后可以在跨渠道投放中定位他们。

   [!DNL :bulb:] [了解如何定义受众](audiences.md)。
...
1. 创建模板
   ...
   [!DNL :arrow_upper_right:]请参阅 [Campaign Classic v7 文档](https://experienceleague.adobe.com/docs/campaign-classic/using/orchestrating-campaigns/orchestrate-campaigns/marketing-campaign-templates.html?lang=zh-Hans#orchestrating-campaigns){target=&quot;_blank&quot;}以了解如何使用活动模板

   [!DNL :arrow_upper_right:] 了解如何在 [Campaign Classic v7文档](https://experienceleague.adobe.com/docs/campaign-classic/using/automating-with-workflows/introduction/building-a-workflow.html?lang=zh-Hans#workflow-templates){target=&quot;_blank&quot;}中配置工作流程模板

1. 配置类型规则

   利用 Campaign 类型规则来筛选、控制和监测投放发送。例如，疲劳规则可控制消息传递的频率和数量，以避免对收件人过度宣传。实施后，类型规则就会在投放中引用。

   [!DNL :arrow_upper_right:] 在 [Campaign Classic v7 文档](https://experienceleague.adobe.com/docs/campaign-classic/using/orchestrating-campaigns/campaign-optimization/about-campaign-typologies.html?lang=zh-Hans#orchestrating-campaigns){target=&quot;_blank&quot;}中了解有关分类和疲劳管理的更多信息
...
1. **仪表板和列表**
   ...
   [!DNL :bulb:]请参阅[此页面](../dev/customize.md#gs-custom-dashboards)以了解详情。
...
[!DNL :bulb:] 请参阅[本节](../connect/integration.md#gs-crm-connectors)以了解如何将您的 CRM 系统连接到 Campaign

**连接到外部数据库**

您可以通过联合数据访问 (FDA) 模块将 Campaign 云数据库连接到外部系统。

[!DNL :bulb:] 请参阅[本节](../connect/integration.md#gs-fda)，了解如何配置 Campaign FDA 模块以定义访问参数
