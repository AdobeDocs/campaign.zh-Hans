...
| deliveryPartRetryDelaySeconds | 重试deliveryPart之前的最小延迟。 这是跨流程和跨容器的。 延迟以秒为单位。 | 60 | 0：立即重试 | 3600：重试非常慢（每次重试间隔1小时） | 1：使重试在繁忙的日志中易于关注。 |
...
| providerIdThreads | 提供程序ID协调的线程数。 每个实例1个线程就足够了。 设置为0将禁用此容器。 | 1 | 0：在此容器上禁用 | 1 | 1 |
