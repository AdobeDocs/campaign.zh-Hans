...
1. 定義預算。 [了解详情](#creating-a-budget)。

1. 定義成本計算方法：為服務提供者定義成本結構。 [了解详情](../campaigns/providers--stocks-and-budgets.md)。

1. 定義行銷活動成本（傳送/任務） ：傳送和任務產生的成本會針對行銷活動範本個別或全域輸入。 [了解详情](../campaigns/marketing-campaign-deliveries.md#compute-costs-and-stocks)。

1. 合併：根據任務、傳遞和行銷活動的進度狀態，將計算成本並傳遞至對應的預算。 當行銷活動的建立足夠進階時，行銷活動預算的進度狀態可以變更為 **[!UICONTROL Specified]**. 然後，系統會自動輸入方案的已計算成本，以及促銷活動上已計算的成本。 [了解详情](#cost-commitment--calculation-and-charging)。
...
1. 輸入初始金額：在相關欄位中指出配置的金額。 其他金額會自動輸入。 [了解详情](#calculating-amounts)。
1. 輸入開始和結束日期，以定義有效期間。 此資訊僅供參考。
1. 建立費用類別，將行銷活動、任務等的成本指派給此預算。 可以連結。 [了解详情](#expense-categories)。

![](assets/s_ncs_user_budget_create_and_save.png)

>[!NOTE]
>
>您可以選取相關預算。 如需详细信息，请参阅[此部分](#linking-a-budget-to-another)。
...
1. 從建立服務提供者及其成本結構的服務範本 **[!UICONTROL Administration > Campaigns]** 節點。 如需详细信息，请参阅[此部分](../campaigns/providers--stocks-and-budgets.md#create-a-service-provider-and-its-cost-categories)。
