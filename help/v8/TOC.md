...
user-guide-description: Adobe Campaign v8 的产品文档（客户端控制台）。
...
+ 发行说明 {#releases}
   ...
   + {hide-from-toc}[已知问题](start/known-issues.md)
   + [兼容性矩阵](start/compatibility-matrix.md)
   + {hide-from-toc}[文档更新](start/documentation-updates.md)
+ 快速入门 {#new}
   ...
   + {hide-from-toc}[常见问题解答](start/campaign-faq.md)
   + {hide-from-toc}[电子邮件跟踪像素和CNIL指南](start/cnil-pixel-tracking.md)
+ 管理和权限 {#permissions}
...
+ 发送消息 {#send}
   ...
   + 短信 {#sms}
      ...
      + [关于SMS连接器类型](send/sms/sms-connectors.md)
      ...
      + 配置短信渠道 {#config-sms}
         ...
         + {hide-from-toc}[单独实例](send/sms/sms-standalone-instance.md)
...
+ 轮廓和受众 {#audience}
   ...
   + [使用过滤器](audiences/create-filters.md)
   + 添加轮廓 {#add-profiles}
   ...
   + 创建受众 {#create-audiences}
      ...
      + {hide-from-toc}[与Adobe解决方案共享受众](start/shared-audiences.md)
   + {hide-from-toc}[最佳做法](audiences/audiences-best-practices.md)
...
+ 配置 {#config}
   ...
   + 设置 {#settings}
      ...
      + {hide-from-toc}[将Campaign SDK与您的应用程序集成 — 已弃用页面](config/push-config.md)
...
+ 开发人员资源 {#developer}
   ...
   + {hide-from-toc}[使用queryDef查询数据库](dev/query-api.md)
   + REST API {#apis}
