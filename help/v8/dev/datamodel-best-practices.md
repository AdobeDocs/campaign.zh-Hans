...
除了在大多数表中默认定义的&#x200B;**autouuid**&#x200B;之外，您还应考虑添加一些逻辑或业务密钥（帐号、客户端号等）。 它稍后可用于导入/协调或数据包。 有关更多信息，请参阅[Identifiers](#identifiers)。
...
Adobe Campaign主键是为所有内置表自动生成的UUID。 UUID也可用于自定义表。
...
* 使用主键作为外部系统主键。 此解决方案通常比较首选，因为它通过不同系统之间一致的键简化了数据导入和导出的方法。 如果键名为“id”，并且应使用外部值（而非自动生成）填充，则应禁用Autouuid。
...
将链接声明为外部连接不利于性能。 零ID记录模拟外部连接功能。 如果链接使用autouuid，则无需声明外部连接。
