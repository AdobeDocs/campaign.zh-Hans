...
>[!CAUTION]
>
>某些内置Campaign架构在云数据库上具有关联的架构。 这些架构由&#x200B;**Xxl**&#x200B;命名空间标识，不得修改或扩展。
...
某些命名空间是保留空间，用于描述Adobe Campaign应用程序操作所需的系统实体。 不得使用下列命名空间&#x200B;**来标识任何大写/小写组合的新架构：**
...
有关详细信息，请参阅[创建新架构](create-schema.md)。
...
每个表必须至少有一个键，通常使用设置为&#x200B;**true**&#x200B;的&#x200B;**autopk**&#x200B;属性在架构的主元素中自动建立该键。

此外，在[企业(FFDA)部署](../architecture/enterprise-deployment.md)的上下文中，使用&#x200B;**@autouuid**&#x200B;并将其设置为&#x200B;**true**。
...
在此示例中，我们指定自己的“householdId”主键，而不是让&#x200B;**@autopk**&#x200B;或&#x200B;**@autouuid**&#x200B;属性创建一个名为“id”的默认主键。

>[!CAUTION]
>
>创建新架构或在架构扩展期间，您需要为整个架构保留相同的主键序列值(@pkSequence)。

在[本节](database-mapping.md#management-of-keys)中了解有关密钥的更多信息。

## 属性（字段） {#attributes--fields-}

属性允许您定义组成数据对象的字段。 您可以使用架构版工具栏中的&#x200B;**[!UICONTROL Insert]**&#x200B;按钮，将空属性模板拖放到光标所在的XML中。 可在[此部分](create-schema.md)中了解详情。
...
有关每个属性的详细信息，请参阅[Campaign Classicv7文档](https://experienceleague.adobe.com/docs/campaign-classic/using/configuring-campaign-classic/schema-reference/elements-attributes/schema-introduction.html#configuring-campaign-classic){target="_blank"}中的属性说明。
...
>[!NOTE]
>
>当修改不会影响数据库结构时，只需重新生成模式即可。 为此，请选择要更新的架构，右键单击并选择&#x200B;**[!UICONTROL Actions > Regenerate selected schemas...]**。
