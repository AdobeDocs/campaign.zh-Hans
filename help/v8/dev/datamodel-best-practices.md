...
要更好地了解活动内置表及它们之间的关系，请参阅[本节](datamodel.md)。
...
* **expr**&#x200B;属性允许将模式属性定义为计算字段，而不是表中的物理集值。 这可以允许以不同格式（例如，年龄和出生日期）访问信息，而无需存储这两个值。 这是避免重复字段的好方法。 例如，收件人表使用域的表达式，该域已在电子邮件字段中显示。
...
如果字段具有定位或个性化目的，则需要将其存储在表中。 换句话说，如果某个字段不用于发送个性化电子邮件或用作查询中的标准，将不必要地占用磁盘空间。
...
Adobe Campaign主键是所有内置表的自动生成的UUID。 UUID也可用于自定义表。
...
谨防大桌上的“自身”诚信。 删除具有“自有”完整性的大表的记录可能会潜在地停止实例。 表已锁定，删除操作逐个完成。 因此，最好在具有大卷的子表上使用“中立”完整性。
