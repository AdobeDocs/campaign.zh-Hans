...
要更好地了解Campaign内置表及其相互关系，请参阅 [本节](datamodel.md).

![](../assets/do-not-localize/glass.png) 已阅读 [本节](schemas.md) 以开始使用Campaign模式。

![](../assets/do-not-localize/glass.png) 了解如何配置扩展模式以扩展Adobe Campaign数据库的概念数据模型 [此页面](extend-schema.md).
...
>[!NOTE]
>
>Adobe Campaign允许构建 [自定义收件人表](custom-recipient.md). 但是，在大多数情况下，建议利用内置 [收件人表](datamodel.md#ootb-profiles) 已经预建了附加表和功能。
...
* 此 **表达式** 属性允许将模式属性定义为计算字段，而不是表中的物理集值。 这可以启用以不同格式（例如年龄和出生日期）访问信息的功能，而无需存储这两个值。 这是避免字段重复的好方法。 例如，收件人表使用域表达式，该表达式已存在于电子邮件字段中。
* 但是，当表达式计算复杂时，建议不要使用 **表达式** 属性，因为动态计算可能会影响查询的性能。
* 此 **XML** 键入是避免创建太多字段的好方法。 但是，它也会占用磁盘空间，因为它使用数据库中的CLOB列。 它还会导致复杂的SQL查询，并可能影响性能。
* 的长度 **字符串** 字段应始终使用列进行定义。 默认情况下，Adobe Campaign中的最大长度为16K，但Adobe建议，如果您已经知道该大小不会超过更短的长度，请缩短该字段。
* 如果您确定源系统中的字段大小被高估并且无法到达，则可以接受Adobe Campaign中的字段比源系统中的字段更短。 这可能意味着Adobe Campaign中的字符串长度更短，整数更小。

### 字段选择 {#choice-of-fields}

如果某个字段具有定位或个性化目的，则需要将其存储在表中。 换言之，如果某个字段未用于发送个性化电子邮件或未用作查询中的标准，则它将不必要地占用磁盘空间。

### 键选择 {#choice-of-keys}

除了 **autouuid** 和 **autopk** 在大多数表中默认定义时，应考虑添加一些逻辑或业务密钥（帐号、客户机号等）。 以后可用于导入/协调或数据包。 有关此内容的更多信息，请参阅 [标识符](#identifiers).

高效的密钥对于性能至关重要。 通过Snowflake，您可以将数字或基于字符串的数据类型作为表的键插入。
...
| 标识符 | 说明 | 最佳实践 |
|--- |--- |--- |
| Id | <ul><li>ID是Adobe Campaign表的物理主键。 对于内置表，它是一个通用唯一ID (UUID)</li><li>此标识符必须是唯一的。 </li><li>UUID可以在架构定义中可见。</li></ul> | <ul><li>不应将自动生成的标识符用作工作流或包定义中的引用。</li><li>表格中的ID是UUID，不应更改此类型。</li></ul> |
| 名称（或内部名称） | <ul><li>此信息是表中记录的唯一标识符。 此值可以手动更新，通常使用生成的名称。</li><li>当此标识符部署在Adobe Campaign的其他实例中时，会保留其值，并且它不应为空。</li></ul> | <ul><li>如果打算将对象从一个环境部署到另一个环境，请重命名Adobe Campaign生成的记录名称。</li><li>当对象具有命名空间属性(*架构* 例如)，此通用命名空间将在创建的所有自定义对象中使用。 不应使用某些保留的命名空间： *nms*， *xtk*&#x200B;等。  请注意，某些命名空间仅是内部命名空间。 [了解详情](schemas.md#reserved-namespaces)。</li><li>当对象没有任何命名空间时(*工作流* 或 *投放* 例如)，此命名空间概念将添加为内部名称对象的前缀： *namespaceMyObjectName*.</li><li>请勿使用空格“ ”、半列“：”或连字符“ — ”等特殊字符。 所有这些字符都将替换为下划线“_”（允许的字符）。 例如，“abc-def”和“abc：def”将存储为“abc_def”并相互覆盖。</li></ul> |
...
* 使用主键作为外部系统主键。 此解决方案通常为首选方案，因为它简化了导入和导出数据的方法，并在不同系统之间使用了一致的密钥。 **Autuuid** 如果键名为“id”，并且应使用外部值填充（非自动生成），则应禁用该键。
...
请注意大型表上的“自身”完整性。 删除具有“自有”完整性的大型表的记录可能会停止实例。 表格被锁定，删除操作逐一进行。 因此，最好在具有大卷的子表上使用“中立”完整性。

将链接声明为外部连接对性能不利。 零ID记录模拟外部连接功能。 在上下文中 [企业(FFDA)部署](../architecture/enterprise-deployment.md)，如果链接使用 **autouuid**.
...
![](../assets/do-not-localize/speech.png)  作为托管Cloud Services用户，请联系Adobe顾问或技术管理员，了解有关保留的详细信息，或者您是否需要为自定义表设置保留。
