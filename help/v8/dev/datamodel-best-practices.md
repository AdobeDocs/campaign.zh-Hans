...
| 名稱（或內部名稱） | <ul><li>此資訊是表格中記錄的唯一識別碼。 此值可以手動更新，通常使用產生的名稱。</li><li>此識別碼在Adobe Campaign的其他執行個體中部署時保留其值，且不應空白。</li></ul> | <ul><li>如果物件要從某個環境部署至另一個環境，請重新命名Adobe Campaign產生的記錄名稱。</li><li>當物件具有名稱空間屬性(*綱要* 例如)，此通用名稱空間將在所有建立的自訂物件間運用。 部分保留的名稱空間不應使用： *nms*， *xtk*&#x200B;等。  請注意，某些名稱空間僅供內部使用。 [了解详情](schemas.md#reserved-namespaces)。</li><li>當物件沒有任何名稱空間時(*工作流程* 或 *傳遞* 例如)，此名稱空間概念會新增為內部名稱物件的前置詞： *namespaceMyObjectName*.</li><li>請勿使用特殊字元，例如空格「 」、半欄「：」或連字型大小「 — 」。 所有這些字元都會取代為底線「_」（允許的字元）。 例如，「abc-def」和「abc：def」會儲存為「abc_def」並互相覆寫。</li></ul> |
| 标签 | <ul><li>標籤是Adobe Campaign中物件或記錄的商業識別碼。</li><li>此物件允許空格和特殊字元。</li><li>它無法保證記錄的唯一性。</li></ul> | <ul><li>建議決定物件標籤的結構。</li><li>這是為Adobe Campaign使用者識別記錄或物件的最好記解決方案。</li></ul> |

在的內容中 [企業(FFDA)部署](../architecture/enterprise-deployment.md)，Adobe Campaign主索引鍵為所有內建表格自動產生的UUID。 UUID也可用於自訂表格。 [了解详情](../architecture/keys.md)

即使ID的數量是無限的，您也應該注意資料庫的大小，以確保最佳效能。 若要避免發生任何問題，請務必調整執行個體清除設定。 有关更多信息，请参阅[此章节](#data-retention)。
