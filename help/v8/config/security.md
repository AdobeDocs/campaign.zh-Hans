...
* [实例和网络安全](#instance-and-network-security)： IP、URL权限、通过控制面板的GPG密钥
...
隐私管理是指可帮助您遵守隐私法规（GDPR 和 CCPA 等）的所有流程和工具。
...
通过Adobe Campaign，您可以跟踪已发送的电子邮件和投放收件人的行为：打开、单击链接、取消订阅等。有关此内容的详细信息，请参阅[关于邮件](../start/gs-message.md)。
...
* **IP 允许列表** — 管理IP以访问实例；限制到已知网络（例如，办公室、VPN），并尽可能避免范围过于广泛。
