...
* [动态报告](https://experienceleague.adobe.com/docs/campaign-web/v8/reports/dynamic-reporting/get-started-reporting.html?lang=zh-Hans){target="_blank"}（仅按需）
* [集中品牌](https://experienceleague.adobe.com/docs/campaign-web/v8/conf/branding/branding-gs.html?lang=zh-Hans){target="_blank"}（仅限点播）
...
* setup-client.exe文件已替换为ac-client.msi文件。 这为管理员提供了一种无需用户干预即可执行批量升级的更简单方法。
