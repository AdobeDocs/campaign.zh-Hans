...
1. 使用“+”符号选择要分配给用户的产品配置文件或用户组。
   ...
   中列出了营销活动内置产品配置文件 [本节](#ootb-productprofiles).
...
新用户时 [登录Campaign](connect.md) 使用它们的Adobe ID，它们会添加到客户端控制台的Campaign运算符列表中。 Campaign运算符存储在 **[!UICONTROL Administration > Access management > Operators]** Campaign资源管理器的文件夹。
...
1. 单击 **[!UICONTROL New]**，并定义运算符组的名称以及设置其内部名称（“审阅者”）。
...
1. 从 **产品和服务** 打开促销活动产品。
1. 单击 **新建配置文件** 并输入要创建的产品配置文件的名称，该名称具有所解释的准确语法 [此处](#product-profiles). 例如，我们输入：campaign - `<your-instance-name>`  — 审阅者

   ![](assets/new-product-profile-ui.png)

1. 保存您的更改。

您现在可以将用户添加到此新的产品用户档案，如中所述 [本节](#add-users).
...
这些产品配置文件与Campaign匹配 **操作员组**. 默认运算符组及其 [已命名权限](#use-named-rights) 如下所示：
...
1. **[!UICONTROL Content contributors]** （内容）
   ...
   此组中的用户可以访问上下文中的内容文件夹 **[!UICONTROL Content management]** 加载项。 此组不授予任何其他权限。
...
1. **[!UICONTROL Offer managers]** (优惠)
   ...
   * 批准选件。
   ...
   >[!NOTE]
   >
   >**优惠经理** 仅当未指定审阅人或在选件模板中已将他们设置为审阅人时，才能批准选件。
...
Adobe Campaign提供了一组已命名权限，允许您定义分配给用户和用户组的权限。 这些权限可从以下位置编辑： **[!UICONTROL Administration > Access management > Named rights]** Campaign资源管理器的文件夹。
...
* 对文件夹的访问操作员组的成员资格可以通过更改文件夹的安全性设置来授予或限制对文件夹的访问权限。 [了解详情](folder-permissions.md#restrict-access-to-a-folder)。
  ...
  例如，它可以影响： **写入权限** 要创建新实体（例如投放、用户档案等），请执行以下操作： **读取权限** 要使用实体，请执行以下操作 **删除访问权限** 以删除实体。
...
* **[!UICONTROL WORKFLOW]**：此已命名权限特定于工作流：它允许您创建、启动和停止工作流。 已命名权限需要工作流文件的读取权限才能适用。 对于定位工作流，请阅读以下页面上的 **[!UICONTROL Profiles and Targets]** 文件夹是必需的。
